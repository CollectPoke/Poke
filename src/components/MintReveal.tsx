  ...
  return (
    <div role="dialog" aria-modal="true" aria-labelledby="mint-reveal-title" className="mint-reveal fixed inset-0 z-[100] flex items-center justify-center overflow-hidden px-5 py-6">
      ...
      <div className="relative flex h-[min(800px,95dvh)] w-full max-w-sm flex-col items-center justify-center text-center">
        <p className="mint-reveal-kicker font-display text-sm font-bold uppercase tracking-[0.2em] text-brand">Pump.fun launch confirmed</p>
        ...
        <div className="mint-reveal-card my-5 w-[min(72vw,280px)]"><NftCard card={card} /></div>
        <div className="mint-reveal-copy">
          ...
          <p className="mt-1 text-sm text-primary-foreground/65">${card.ticker} is live on Solana and you claimed its one-of-one JPEG.</p>
          <a href={`https://solscan.io/tx/${signature}`} target="_blank" rel="noreferrer" className="mt-3 block text-xs font-semibold text-brand underline underline-offset-4">View launch on Solscan ↗</a>
          <Button onClick={onContinue} className="mt-5 h-12 w-full rounded-full bg-brand font-display font-bold text-brand-foreground hover:bg-brand/90">See my new NFT</Button>
        </div>
      </div>
    </div>
  );
