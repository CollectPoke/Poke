...
function CardPage() {
  ...
  if (isLoading) {
    return (
      <main className="mx-auto max-w-5xl px-5 py-20 text-sm text-muted-foreground">Loading…</main>
    );
  }
  if (!card) {
    return (
  ...
  return (
    <>
        ...
        <div className="mt-4 grid gap-8 md:grid-cols-[320px_1fr]">
          ...
          <div>
            ...
            <div className="mt-6 rounded-2xl border border-border bg-secondary p-4">
              ...
              <p className="mono-num mt-1 break-all text-xs text-brand">{card.contract_address}</p>
              <div className="mt-3 flex flex-wrap gap-4 text-xs font-semibold">
                <a
                  href={`https://pump.fun/coin/${card.contract_address}`}
                  target="_blank"
                  rel="noreferrer"
                ...
                {card.launch_tx_signature ? (
                  <a
                    href={`https://solscan.io/tx/${card.launch_tx_signature}`}
                    target="_blank"
                    rel="noreferrer"
                    className="text-brand underline underline-offset-4"
                  >
            ...
            <div className="mt-6 rounded-2xl border border-border bg-card p-5 shadow-card">
              {!user ? (
                ...
                <p className="text-sm text-muted-foreground">This NFT is not for sale right now.</p>
              )}
              {error && <p className="mt-3 text-sm font-medium text-danger">{error}</p>}
              {burnNote && <p className="mt-3 text-sm font-medium text-success">{burnNote}</p>}
              {txSig && (
                <a
            ...
            <div className="mt-2 rounded-2xl border border-border bg-card p-4">
              {owners.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  {user
                    ? "Still with its original minter — never sold."
                    : "Sign in to see the owner history."}
                </p>
              ) : (
      ...
      {showReveal && txSig ? (
        <PurchaseReveal
          card={{ ...card, owner_id: user?.id ?? card.owner_id, owner: { username: "you" } }}
          signature={txSig}
